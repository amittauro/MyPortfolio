...
directions = ['passage']
while directions[-2..-1] != ['cave', 'light']
  player = gets.chomp
  if directions[-1] == 'passage' and player == 'north'
    puts 'You are in a scary cave.'
    directions.push('cave')
  elsif directions[-1] == 'cave' and player == 'south'
    puts 'You are in a scary passage.'
    directions.push('passage')
  elsif player != 'north' and player != 'south'
    next
  elsif directions[-1] == 'cave' and player == 'north'
    puts 'You walk into sunlight.'
    directions.push('light')
    break
  else
    next
  end
