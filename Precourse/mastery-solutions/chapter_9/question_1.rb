...
def score(list)
  points = { "two" => 2, "three" => 3, 'four' => 4, 'five' => 5, 'six' => 6, 'seven' => 7,
   'eight' => 8, 'nine' => 9, 'ten' => 10, 'jack' => 10, 'queen' => 10, 'king' => 10, 'ace' => 11 }
  list.map { |x| points.fetch(x) }.reduce(0, :+)
end

def run_game
  list = []
  while move == 'hit'
    list.push(random_card)
    puts "Score so far: #{score(list)}"
  end
  if score(list) <= 21
    puts "You scored: #{score(list)}"
  else
    puts "You busted with: #{score(list)}"
  end
end
